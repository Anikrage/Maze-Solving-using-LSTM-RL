...
if st.button("Generate Maze"):
    ...
    ax.scatter(st.session_state.env.start[1], st.session_state.env.start[0], c="blue", marker="o", label="Start")
    ax.scatter(st.session_state.env.goal[1], st.session_state.env.goal[0], c="red", marker="x", label="Goal")
    ax.legend()
    st.pyplot(fig)
    st.session_state.agent_trained = False
    st.session_state.model = None
